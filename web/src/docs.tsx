...
is yours to change, under [Command replies](tab:messages); there is no second system message
...
The button only appears once [remote access](#remote) is on — before that the console is reachable
...
| **When a tool needs the PIN** | a gated tool call is waiting on the [PIN](#settings) | \`{tool}\`, \`{seconds}\` |
...
[remote access](#remote) is on, so the person it happened to can send you the failure with its logs —
see [Feedback](#settings).
`,
  },
  {
    id: 'keys',
...
| --- | --- | --- |
| Slow replies, or "I'm a bit rate-limited" | Free-tier rate limiting — every model busy at once | Wait a minute; on [Behavior](tab:behavior), start the chain at a less-contended model |
| "My mind went blank" | The model returned nothing usable — the cause is in the logs, not the quota | Use the **Report this** button on the message, which sends the team that failure and its logs ([Feedback](#settings)) |
